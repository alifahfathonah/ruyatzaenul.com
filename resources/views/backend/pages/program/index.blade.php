...
@section('title')
	<title>Program | Official Website Ru'yat-Zaenul</title>
@stop
...
function loadData(id)
{
  ...
  $('div#data').load(APP_URL+'/program-data/'+id,function(){
    ...
    $('table#data-berita').DataTable({
		...
		$('.switch').on('switchChange.bootstrapSwitch', function (event, state) {
			...
			$.ajax({
					dataType: 'json',
					type:'POST',
					url: APP_URL+'/program-status/'+iduser,
					data:{flag : st},
					...
					// getPageData();
					var txt = "Status Program Berhasil Di Edit";
				...
				new PNotify({
						title: 'Informasi',
						text: 'Edit Status Program Gagal',
...
<ul class="fab-menu fab-menu-fixed fab-menu-bottom-right" data-fab-toggle="hover">
    <li>
      ...
      <ul class="fab-menu-inner">
				<li>
					<div data-fab-label="Tambah Program">
						<a href="{{URL::to('/program-form/-1')}}" class="btn btn-default btn-rounded btn-icon btn-float">
