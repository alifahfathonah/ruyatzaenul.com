...
@section('title')
	<title>Slider | Official Website Ru'yat-Zaenul</title>
@stop
...
$(document).ready(function(){
  ...
  loadData(-1);
	...
	$('button#confirmbutton').click(function(){
		...
		if(title=='')
		{
			$('#titlex').text('Nama Slider Harus Diisi');
			setTimeout(function(){
				$('#titlex').text('');	
			},3000);
		}
		...
		{
			$('#thumbnailx').text('Gambar Slider Belum Dipilih');
			setTimeout(function(){
				$('#thumbnailx').text('');	
			},3000);
		}
		...
		{
			$('#flagx').text('Status Slider Belum Dipilih');
			setTimeout(function(){
				$('#flagx').text('');	
			},3000);
		}
		...
		{
			$('#descx').text('Keterangan Slider Belum Diisi');
			setTimeout(function(){
				$('#descx').text('');	
			},3000);
		}
		...
		else
		{
			$.ajax({
					...
					type:form_method,
					url: form_action,
					data:$('form#form-slider').serialize()
			}).done(function(data){
					// getPageData();
					if(form_method=="PUT")
						var txt = "Data Slider Berhasil Di Edit";
					else
						var txt = "Data Slider Berhasil Di Tambah";
...
function loadData(id)
{
  ...
  $('div#data').load(APP_URL+'/slider-data/'+id,function(){
    $('div#data-loader').hide();
    $('table#data-slider').DataTable({
			autoWidth: false,
		...
		$('.switch').on('switchChange.bootstrapSwitch', function (event, state) {
			...
			$.ajax({
					dataType: 'json',
					type:'PUT',
					url: APP_URL+'/slider/'+iduser,
					data:{flag : st}
			}).done(function(data){
					// getPageData();
						var txt = "Status Slider Berhasil Di Edit";
...
function hapus(id)
{
	...
	$('#hapusbutton').click(function(){
        ...
        $('#modal-hapus').modal('hide');
				    new PNotify({
		            title: 'Berhasil',
		            text: 'Data Slider Berhasil Di Hapus',
		            addclass: 'alert bg-success alert-styled-right',
		            type: 'success'
		        });
						loadData(-1);
...
function form(id)
{
	...
	$('div#dynamic-content-confirm').load(APP_URL+'/slider-form/'+id,function(){
